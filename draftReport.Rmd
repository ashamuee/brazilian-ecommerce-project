...
```{r Prepare final wrangled data set, echo=FALSE}
...
wrangled_data <- wrangled_data %>% cbind.data.frame(product_category_dummyvars) %>% select(-product_category_name)
...
rm(geolocation_dataset, order_items_dataset, orders_dataset, p1, product_category_name_translation, products_dataset, sellers_dataset, sellers_to_ignore, customers_dataset, customers_to_ignore, raw_data)
```
...
get_delay_in_handover_to_carrier_per_seller_id <- function(df) {
  df %>% mutate(delay= (as.numeric(handed_over_to_carrier_date) -as.numeric(shipping_limit_date))/3600)  %>% 
    group_by(seller_id) %>% 
    summarise(mean_penalty_to_consider=mean(delay))
}
```
...
```{r Split data into training and validation set, include = FALSE}
set.seed(1,sample.kind = 'Rounding')
...
# validation_dataset <- validation_dataset %>% semi_join(training_dataset,by='product_category_name') %>% semi_join(training_dataset,by='seller_id')
...
# TODO review it, fix it as required
...
train_subset <- train_subset %>% left_join(penalty_data,by='seller_id') %>% select(-handed_over_to_carrier_date, -shipping_limit_date, -seller_id)

test_subset <- test_subset %>% left_join(penalty_data,by='seller_id') %>% select(-handed_over_to_carrier_date, -shipping_limit_date, -seller_id)
```
...
knn_fit <- train(number_of_hours~., train_subset, method='knn',tuneGrid=data.frame(k=c(27,34,41)),trControl=tuningControl)
...
```{r knn model data scaline under test, echo=FALSE}
# TODO review and improve or remove this as required
set.seed(seed=1,sample.kind = 'Rounding')
scaled_train_subset <- train_subset %>% mutate(across(c(mean_penalty_to_consider,distance_between_two_points,volume,product_weight_g,freight_value,price),scale)) 
scaled_test_subset <- test_subset %>% mutate(across(c(mean_penalty_to_consider,distance_between_two_points,volume,product_weight_g,freight_value,price),scale))
...
RMSE(predictions,(test_subset %>% pull(number_of_hours) ))
# 184.1709 for mtry 15. Technically there are total of 79 variables due to 73 product categories and 6 other predictors
...
13  46.9         17.63        sports_leisure              300        354.7378   2304
   distance_between_two_points mean_penalty_to_consider
1                       469.74                -51.60514
2                       469.74                -51.60514
3                       469.74                -51.60514
4                       469.74                -51.60514
