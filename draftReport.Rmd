...
order_items_dataset <- read.csv(file.path('data','olist_order_items_dataset.csv'),header=TRUE,stringsAsFactors = FALSE) %>% mutate(order_id=as.factor(order_id),product_id=as.factor(product_id),seller_id=as.factor(seller_id), shipping_limit_date=as_datetime(shipping_limit_date))
...
raw_data <- raw_data %>% select(shipping_limit_date,handed_over_to_carrier_date) %>% mutate(delayed_to_carrier=if_else((as.numeric(handed_over_to_carrier_date)-as.numeric(shipping_limit_date))>0,TRUE,FALSE))
