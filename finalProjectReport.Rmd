...
This was a crucial dataset not only for us to build our delivery prediction system but to even identify the current performance metric i.e. RMSE for the approach Olist was using.
...
To understand its influence on the delivery timelines, we plotted numbers of hours taken for delivery against the price of the ordered product. In order to see the trend, gam method based smoothing curve was plotted on the graph.

```{r Explore price relation with number_of_hours, echo=FALSE, message=FALSE}
training_dataset  %>% ggplot(aes(price,number_of_hours)) + geom_smooth() + xlab('Price of ordered product') + ylab('Number of hours taken for delivery')
```
In above plot, we noted that number of hours taken for delivery vary as the price of the ordered product changes. It appeared to increase significantly as the price of ordered product increased and decreased when the price reached towards higher limits. Though confidence interval indicated higher variability in delivery timelines at higher prices.
...
if(file.exists('rda/knn_model.rda')) { 
  load('rda/knn_model.rda'); 
  print(knn_fit);
} else {
  lines <- readLines('captured-outputs/knn_model_output.txt')
  cat(lines,sep="\n")
...
```{r print plot for knn model based on train_subset, echo=FALSE, out.width ="100%", fig.align='center'}
if(file.exists('rda/knn_model.rda')) { 
    knn_fit %>% ggplot() 
} else {
  knitr::include_graphics('captured-outputs/knn_model_plot.png')
}
```

Based on model results, **k** value of **`r if(exists("knn_fit",mode="list")) { knn_fit$finalModel$k } else {77}`** was identified to be the one with lowest RMSE.
...
if(file.exists('rda/rf_model.rda')) { 
  load('rda/rf_model.rda'); 
  print(rf_fit);
} else {
  lines <- readLines('captured-outputs/rf_model_output.txt')
  cat(lines,sep="\n")
...
```{r print plot for rf model based on train_subset, echo=FALSE, out.width ="100%", fig.align='center'}
if(file.exists('rda/rf_model.rda')) { 
    rf_fit %>% ggplot() 
} else {
  knitr::include_graphics('captured-outputs/rf_model_plot.png')
}
```
...
Based on model results, **mtry** value of **`r if(exists("rf_fit",mode="list")) { rf_fit$finalModel$mtry } else {2}`** was identified to be the one with lowest RMSE.
...
if(file.exists('rda/rf_model.rda')) { 
  load('rda/rf_model.rda');
  mtry_value = rf_fit$finalModel$mtry
} 

data.frame(tuning_parameter='mtry', value=mtry_value) %>% add_row(tuning_parameter='number_of_trees', value=500) %>% knitr::kable(caption='Random forest tuning parameters')
...
Details of the final trained random forest model can be found below:

```{r trained final random forest model based on training_dataset, echo=FALSE}
if(file.exists('rda/final_model.rda')) {
  load('rda/final_model.rda')
  sink('captured-outputs/final_model_output.txt')
  print(final_model)
  sink()
}

lines <- readLines('captured-outputs/final_model_output.txt')
cat(lines,sep="\n")

if(exists("final_model",mode="list")) {
  plot(final_model,main='Random forest model error against number of trees')
} else {
  plot_object <- readRDS('rds/final_model_plot.rds')
  plot(plot_object,type='l',main='Random forest model error against number of trees', xlab='trees', ylab='Error')
...
# Conclusion
As part of this project, the goal was to build a delivery prediction system which could predict the number of hours required to delivered the ordered product rating with minimal RMSE. 
