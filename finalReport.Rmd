...
```{r smooth fit for price, echo=FALSE}
training_dataset %>% ggplot(aes(x=price,y=number_of_hours)) + geom_smooth() 
```
...
```{r Post regularization for product_category effect, echo=FALSE}
...
train_subset <- train_subset %>% left_join(product_category_effects,by='product_category_name') %>% select(-product_category_name) %>% mutate(product_category_effect=if_else(is.na(product_category_effect),global_mean,product_category_effect))
      
test_subset <- test_subset %>% left_join(product_category_effects,by='product_category_name') %>% select(-product_category_name)
```
...
train_subset <- train_subset %>% left_join(seller_id_effects,by='seller_id') %>% select(-seller_id) %>% mutate(seller_id_effect=if_else(is.na(seller_id_effect),global_mean,seller_id_effect))
      
test_subset <- test_subset %>% left_join(seller_id_effects,by='seller_id') %>% select(-seller_id)
```
...
cor(train_subset %>% select(price,freight_value,product_category_effect,customer_loc_effect,customer_state_effect,seller_id_effect,distance_between_two_points,mean_delay_in_handover,volume,number_of_hours,product_weight_g),use="pairwise.complete.obs") %>% knitr::kable()
```
...
set.seed(seed=1,sample.kind = 'Rounding')
tuningControl=trainControl(method = "cv", number = 2, p = .8)
rf_fit <- train(number_of_hours~., train_subset, method='rf',tuneGrid = data.frame(mtry = c(1,2,3,4,5,10)),trControl=tuningControl,ntree=250)
...
# 173.9907 for mtry 2. Technically there are total of 10 predictors. The seller_state was not of much help. it was actually increasing RMSE. So have ignored it.
...
RMSE(predictions,(scaled_test_subset %>% pull(number_of_hours) ))
# 183.4923 uses GCV.Cp method and select=TRUE
...
enhance_datasets_using_product_category_effect <- function(product_category_effects,train_df,test_df,cleanup) {
  ...
  train_df <- train_df %>% left_join(product_category_effects,by='product_category_name') %>% mutate(product_category_effect=if_else(is.na(product_category_effect),global_mean,product_category_effect))
...
```{r Final model analysis, echo=FALSE}
validation_dataset %>% select(number_of_hours) %>% mutate(diff=(number_of_hours-predictions)) %>% ggplot(aes(diff)) + geom_histogram(binwidth = 1) + scale_x_log10()

validation_dataset %>% select(number_of_hours) %>% mutate(diff=number_of_hours-predictions) %>% summary()

```



```{r Final knn model traing with data scaled, echo=FALSE}
set.seed(seed=1,sample.kind = 'Rounding')
