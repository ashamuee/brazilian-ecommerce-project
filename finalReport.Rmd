...
raw_data <- orders_dataset %>% 
  ...
  select(-customer_id,-order_status,-seller_city,-customer_city, -payment_approved_at,-order_id,-order_item_id,-product_id,-seller_zip_code_prefix,-customer_unique_id,-customer_zip_code_prefix)
...
```{r Apply Target encoding for customer_state, echo = FALSE}
  train_subset_for_encoding <- train_subset[1:40000,] %>% select(-seller_state)
  global_mean <- mean(train_subset_for_encoding$number_of_hours)
...
```{r Apply Target encoding for seller_state, echo = FALSE}
  ...
  test_weight_set <- test_weight_set %>%
    #semi_join(train_weight_set,by='product_category_name') %>%
    semi_join(train_weight_set,by='seller_state') 
  
  weight_rmses <- sapply(weights, function(weight) {
      seller_state_effects <- train_weight_set %>% 
        group_by(seller_state) %>% 
        summarise(mean=mean(number_of_hours),n=n()) %>%
        mutate(mean=(n*mean+weight*global_mean)/(n+weight)) %>% 
        select(-n) %>% rename()
      
      train_weight_set <- train_weight_set %>% left_join(seller_state_effects,by='seller_state') %>% rename(seller_state_effect=mean) %>% select(-seller_state) #-product_category_name
      
      test_weight_set <- test_weight_set %>% left_join(seller_state_effects,by='seller_state') %>% rename(seller_state_effect=mean) %>% select(-seller_state) # -product_category_name,
...
```{r Post regularization with seller state effect, echo=FALSE}
...
seller_state_effects_list <- lapply(1:10, function(index) {
    train_weight_set <- train_subset[-test_weight_indices[,index],]
    ...
    seller_state_effects
  }
)

sse <- seller_state_effects_list[[1]]
for (index in 2:10) {
   sse <- sse %>% full_join(seller_state_effects_list[[index]],by='seller_state', suffix=c("1",index))
}

# whereever value is NA, we have used global mean
sse <- sse %>% mutate(across(-seller_state,~if_else(is.na(.x),global_mean,.x)))

# as different partitions may not have exactly same data set combined the results
seller_state_effects <- tibble(sse[,1], seller_state_effect=rowMeans(sse[,-1],na.rm = TRUE))

train_subset <- train_subset %>% left_join(seller_state_effects,by='seller_state') %>% select(-seller_state) %>% mutate(seller_state_effect=if_else(is.na(seller_state_effect),global_mean,seller_state_effect))
      
test_subset <- test_subset %>% left_join(seller_state_effects,by='seller_state') %>% select(-seller_state)
```
...
#190.243 for k=41. The seller_state is not of much help. it decrease RMSE by .14
...
set.seed(seed=1,sample.kind = 'Rounding')
tuningControl=trainControl(method = "cv", number = 2, p = .8)
rf_fit <- train(number_of_hours~., train_subset, method='rf',tuneGrid = data.frame(mtry = seq(5, 50, 10)),trControl=tuningControl,ntree=250)
...
# 181.8166 for mtry 15. Technically there are total of 82 variables due to 73 product categories and 9 other predictors. The seller_state is not of much help. it increases RMSE by .6. So we should consider ignoring it.
