...
k-nearest neighbors(knn) being based on data points of nearest neighbours was a good starting point. And the fact that knn model does not make assumptions about the dataset, made it a good candidate for initial performance evaluation. 

To use knn model, features were scaled and normalized to prevent overfitting as knn internally calculates Euclidean distance for finding the nearest data-points. 

In addition, for knn model training and tuning, we carried our 5-fold cross validation, which helped to remove any randomness from our tuning parameters. This helped reduced the probability of overfitting.
...
performance_metric
# 173.3481 for mtry 2. Technically there are total of 11 predictors. The seller_state was not of much help. it was actually increasing RMSE. So have ignored it.

save(rf_fit,file='rda/rf_model.rda')
```

==TODO include gam model if required.
...
performance_metric <- performance_metric %>% 
  add_row(type='Ensemble model(partial dataset)',
          value=RMSE(rowMeans(predictions_df),(test_subset %>% pull(number_of_hours) )))
#175.7183

# max approach
RMSE(predictions_df %>% mutate(max=pmax(predictions_knn,predictions_rf)) %>% pull(max),(test_subset %>% pull(number_of_hours)))
#177.7854

# min approach
RMSE(predictions_df %>% mutate(min=pmin(predictions_knn,predictions_rf)) %>% pull(min),(test_subset %>% pull(number_of_hours)))
#178.7218
```
...
# Results
Post model tuning and evaluation, random forest algorithm emerged as the right fit for our data. The tuning parameters identified for the final random forest algorithm are shared below:
...
Below is the summary of model against the performance metric i.e. RMSE
...
# Conclusion
As part of this project, the goal was to build a delivert prediction system which could predict the number of hours required to delivered the ordered item rating with minimal RMSE. 
...
Once the feature analysis and selection was completed, we used the features to tune models like k-nearest neighbours, random forest. We carried out k-fold cross validation to remove any randomness from the model training and tuning process. Finally we reached to a model which performs within desired range of RMSE and can be useful for predicting number of hours required for delivery of a given ordered item. Due care was taken to prevent overfitting from influencing our model.
